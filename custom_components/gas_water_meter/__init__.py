...
def _register_services(hass: HomeAssistant) -> None:
    ...
    async def handle_record_reading(call: ServiceCall) -> None:
        ...
        if image_path is not None:
            ...
            # Save the image
            saved_image_path = await store.async_save_image(
                image_path, entry_id, timestamp_str
            )
            ...
            if meter_reading is None:
                ...
                ocr_result = await hass.async_add_executor_job(
                    read_meter_image, image_path
                )

                if ocr_result.meter_reading is not None:
    ...
    async def handle_set_price(call: ServiceCall) -> None:
        ...
        if valid_from is None:
            valid_from = datetime.now(tz=timezone.utc).strftime("%Y-%m-%d")
    ...
    async def handle_read_meter_image(call: ServiceCall) -> dict[str, Any]:
